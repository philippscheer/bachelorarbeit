...
if __name__ == "__main__":
    ...
    parser.add_argument("-r", "--rounds", type=int, help="Number of rounds every test runs", default=1)
    ...
    parser.add_argument("--hc", action="store_true", help="Run Hill Climbing tests (v1 and v3)")
    parser.add_argument("--oo", action="store_true", help="Run Offering Order test")
    parser.add_argument("--config", help="Config ID", default=None, required=False)
    parser.add_argument("--print-schedule", action="store_true", help="Print the found schedule")
    parser.add_argument(
        "--course-min",
        type=int,
    ...
    with tqdm(config_files, desc="Benchmarking constraint config", position=1, leave=True) as pbar:
        for cfg_path in pbar:
            ...
            if args.course_min and args.course_max:
                C.TOTAL_COURSE_COUNT_CONSTRAINT = SimpleNamespace(min=args.course_min, max=args.course_max)
                logger.info(f"Set TOTAL_COURSE_COUNT_CONSTRAINT to {C.TOTAL_COURSE_COUNT_CONSTRAINT}")
                ...
                while is_valid_schedule(schedule, schedule_complete=True):
                    ...
                    C.TOTAL_COURSE_COUNT_CONSTRAINT = SimpleNamespace(min=len(schedule) + 1, max=999)
                    logger.info(f"trying count {C.TOTAL_COURSE_COUNT_CONSTRAINT}")
                    schedule = test_ilp(offerings)
                    logger.info(f"result {schedule}")
                    logger.info(f"length {len(schedule)}")
                    logger.info(f"is valid? {is_valid_schedule(schedule, schedule_complete=True)}")
                    logger.info(f"score: {get_schedule_mark(schedule)}")

                C.TOTAL_COURSE_COUNT_CONSTRAINT = SimpleNamespace(min=len(previous_schedule), max=999)
                logger.success(f"found longest ilp schedule with length {len(previous_schedule)})")
                logger.success(f"is valid? {is_valid_schedule(previous_schedule, schedule_complete=True)}")
                C.TOTAL_COURSE_COUNT_CONSTRAINT = SimpleNamespace(min=1, max=len(previous_schedule))
            ...
            for i in range(ccc_min, ccc_max):
                C.TOTAL_COURSE_COUNT_CONSTRAINT = SimpleNamespace(min=i, max=i)  # fix course count
                    ...
                    test_results[alg_name].append(
                        {
                            ...
                            "valid": is_valid_schedule(schedule, schedule_complete=True),
                            # run timings
