\subsection{Szenariendefinition}\label{chap:szenariendefinition}

Die Szenarien wurden als isolierte Testfälle konzipiert, um eine kontrollierte Vergleichbarkeit zwischen den Algorithmen zu gewährleisten; eine Kombination mehrerer Szenarien ist durch die zugrundeliegende \textit{Constraint}-Architektur technisch möglich, wurde jedoch bewusst ausgeklammert.
...
\subsubsection{Ähnliche Problemstellungen}

Hinzu kommen klassische \textit{Constraints}, die in \textit{University-Course-Timetabling}-Problemen häufig vorkommen \cite{chen-2021}. Diese \textit{Constraints} betreffen meist die universitäre Planung des Curriculums und die Zuteilung von Kursen zu Räumen, es gibt jedoch auch \textit{Constraints}, die die Erstellung individueller Stundenpläne der Studierenden betreffen:


\begin{itemize}
  \item Studierende müssen von 12:00 bis 13:00 Zeit für ein Mittagessen haben
  \item Pro Tag werden entweder keine oder mindestens 3 Stunden Vorlesungen besucht
  \item Pro Tag dürfen nicht mehr als 8 Stunden Vorlesungen besucht werden
\end{itemize}
...
Zusätzlich zu den oben genannten \textit{Constraints} sollen die Algorithmen auch in Extremfällen hinsichtlich ihrer Performance getestet werden. Daher werden ergänzend folgende Szenarien definiert, die in verwandten Arbeiten nicht explizit untersucht wurden, jedoch zur Vergleichbarkeit zwischen den Algorithmen sinnvoll sind:


\begin{itemize}
  \item Alle Kurse können frei von \textit{Constraints} gewählt werden.
  \item 25\% - 75\% aller Kurse können nicht belegt werden.
  \item 1 - 7 Kurse sind vorgegeben
  \item Keine Kurse zwischen 6:00 morgens und 13:00.
