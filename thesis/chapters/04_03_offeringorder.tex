\subsection{Implementierung des Offering-Order Algorithmus}\label{chap:implementation_offering_order}
...
\begin{enumerate}
    \item Alle \textit{Offerings} innerhalb desselben Planpunkts werden absteigend nach \textit{Mark} sortiert. Innerhalb eines Planpunkts wird somit das Offering mit dem höchsten \textit{Mark} zuerst in Betracht gezogen.
...
\begin{enumerate}
    ...
    \item Für jeden Planpunkt wird versucht, das \textit{Offering} mit der höchsten \textit{Mark} auszuwählen. Bevor die Auswahl getroffen wird, wird geprüft, ob das Hinzufügen des \textit{Offerings} zu dem aktuellen Teilstundenplan zu einer zeitlichen Überschneidung führt oder andere \textit{Constraints} verletzt (\textit{Forward-Checking}).
    \item Führt das Hinzufügen eines \textit{Offerings} zu keinem gültigen Endergebnis in den nachfolgenden Planpunkten, wird die Auswahl rückgängig gemacht und das nächstbeste \textit{Offering} der aktuellen Gruppe wird getestet (\textit{Backtracking}).
\end{enumerate}
